...
func Test_gopathDependencyPackageInfo(t *testing.T) {
	t.Run("TestPeer", func(t *testing.T) {
		deps, err := gopathDependencyPackageInfo(runtime.GOOS, runtime.GOARCH, "github.com/Randyshu2018/fabric/cmd/peer")
		assert.NoError(t, err, "failed to get dependencyPackageInfo")

		var found bool
		for _, pi := range deps {
			if pi.ImportPath == "github.com/Randyshu2018/fabric/cmd/peer" {
				found = true
				break
			}
		}
	...
	t.Run("TestIncomplete", func(t *testing.T) {
		_, err := gopathDependencyPackageInfo(runtime.GOOS, runtime.GOARCH, "github.com/Randyshu2018/fabric/core/chaincode/platforms/golang/testdata/src/chaincodes/BadImport")
